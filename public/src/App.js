...
import {currentUser,signIn, signOut, useAuth, signUp,dbUsers,dbImages,dbChats} from './auth/FirebaseConfig.js';
import {texts} from "./texts/Texts.js";
import {getLanguage,Alert, Toast, Avatar,getColor,getCurrentTime} from "./pages/Utils.js";

import NotFoundPage from "./pages/404-page.js";
import Authtication from "./pages/Authtication.js";
import {Admin,Settings} from './pages/Admin.js';
import SupportChat from "./pages/Support.js";
...
const App = ()=>{
  ...
  return(
    <Main props={states} onChanges={onChanges}/>
  );
}
export default App;
...
const Main = ({props, onChanges}) =>{
  ...
  const [toggle,setToggle] =useState({auth:true});
 ...
 async function handleAction(){
    ...
    <main className={`a_main ${props.theme}`}>
      ...
      <nav className="m_nav nav a_container flex_b_c">
        <div className="left_items padd10-lr">
          <div className="logo m5-lr flex_b_c">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-chat-right-dots" viewBox="0 0 16 16">
              ...
              <path fill="rgba(var(--main-color-rgba),0.20)" d="M5 6a1 1 0 1 1-2 0 1 1 0 0 1 2 0m4 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0m4 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0"/>
            </svg>
            
          <div className="m5-l">I-Chat</div>
          </div>
        </div>
  ...
  <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z"/>
              ...
              <div className="a_c_menu a_container_br br4-a">
                {datas && datas.isAdmin && texts.menuList.menu.map((l, index)=>(<div key={index}  className={`padd6-10`}>{l.label[props.language]}</div>))}
      ...
      {toggle.auth && <Authtication language={props.language} id={id} onAuth={()=>setToggle(prevToggle=>({...prevToggle,auth:false}))} /> 
|| <div>
  
  {mode === "admin" &&
    <SupportChat language={props.language}/> ||
    <Chat language={props.language} id={isAuthenticated} mode={"cabinet"}/>
  }
</div>
